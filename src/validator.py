...
def validate_records(csv_input: str, csv_output: str):
    with open(csv_input, "r") as file_in, open(csv_output, "w", newline="") as file_out:
        reader = csv.DictReader(file_in)
        writer = csv.DictWriter(file_out, fieldnames=reader.fieldnames+["status"])
        writer.writeheader()
...
def validate_ride_id(record: dict) -> str:
    ...
    return VALIDATION_STATUS[0] if not has_spaces else VALIDATION_STATUS[1]
...
def validate_bike_id(record: dict) -> str:
    ...
    if bike_id != "BIKE-" + bike_id.split("-")[0]:
        has_spaces = True
...
def validate_station(record: dict, start_or_end_station: int) -> str:
    ...
    return VALIDATION_STATUS[0] if not needs_cleaning else VALIDATION_STATUS[1]
...
def validate_start_time(record: dict) -> str:
    ...
    try:
        ...
        return VALIDATION_STATUS[1] # readable datetime format but not the correct / standard one, needs cleaning
...
def validate_distance(record: dict) -> str:
